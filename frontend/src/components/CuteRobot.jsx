...
export default function CuteRobot({ enableSpeaking = true }) {
  ...
  useEffect(() => {
    ...
    const fillLight = new THREE.DirectionalLight(0xcc99ff, 0.5);
    ...
    const glowMaterial = new THREE.MeshBasicMaterial({ color: 0xDA70D6 });
    ...
    const smileMaterial = new THREE.LineBasicMaterial({ color: 0xDA70D6, linewidth: 3 });
    ...
    // Speech and animation variables
    let mouthOpenAmount = 0;

    // Text-to-speech function
    const speakWelcome = () => {
      const utterance = new SpeechSynthesisUtterance("Welcome to AI Mock Interview");
      utterance.rate = 0.85;
      utterance.pitch = 1.0;
      utterance.volume = 1;
    ...
    if (enableSpeaking) {
      const speakTimeout = setTimeout(speakWelcome, 1000);
      // No direct cleanup for timeout, but it's good practice if it were an interval
    }
    // Animation
    let mouseX = 0;
    let mouseY = 0;
    ...
    const animate = () => {
      ...
      if (speaking) {
        // Random mouth movement for talking effect
        mouthOpenAmount = Math.sin(Date.now() * 0.02) * 0.15 + Math.random() * 0.1;
        smile.scale.y = 1 + mouthOpenAmount;
        smile.position.y = 1.5 - mouthOpenAmount * 0.1;
      } else {
        // Return to normal
        smile.scale.y += (1 - smile.scale.y) * 0.1;
        smile.position.y += (1.5 - smile.position.y) * 0.1;
      }
